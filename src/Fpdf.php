...
public function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	// Core fonts
	$this->CoreFonts = array('courier','simhei', 'helvetica', 'times', 'symbol', 'zapfdingbats');
	// Scale factor
	if($unit=='pt')
		$this->k = 1;
	elseif($unit=='mm')
...
public function AddPage($orientation='', $size='', $rotation=0)
{
	// Start a new page
	if($this->state==3)
		throw new Exception('The document is closed');
...
public function AddFont($family, $style='', $file='', $dir='')
{
	...
	if(strpos($file,'/')!==false || strpos($file,"\\")!==false)
		throw new Exception('Incorrect font definition file name: '.$file);
	if($dir=='')
		$dir = $this->fontpath;
...
public function Text($x, $y, $txt)
{
	...
	if(!isset($this->CurrentFont))
		throw new Exception('No font has been set');
	$txt = (string)$txt;
...
protected function _beginpage($orientation, $size, $rotation)
{
	...
	{
		if($rotation%90!=0)
			throw new Exception('Incorrect rotation value: '.$rotation);
...
protected function _loadfont($path)
{
	...
	include($path);
	if(!isset($name))
		throw new Exception('Could not include font definition file: '.$path);
	if(isset($enc))
		$enc = strtolower($enc);
	if(!isset($subsetted))
		$subsetted = false;
...
protected function _parsegif($file)
{
	...
	$f = fopen('php://temp','rb+');
	if(!$f)
		throw new Exception('Unable to create memory stream');
	fwrite($f,$data);
	rewind($f);
	$info = $this->_parsepngstream($f,$file);
	fclose($f);
